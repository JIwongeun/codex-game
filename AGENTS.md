...
- 핵심 루프: `WASD` 또는 방향키로 `>_` task node를 이동 → 개발·Codex 작업 로그와 범위 오류를 회피 → 한 번 피격 시 종료 → 생존 기록 갱신
- 시각 방향: Codex task surface를 연상시키는 흰 화면, 검정·회색만 사용, Pretendard Variable, 사각형과 선·문자·패턴 중심
