...
- 특수 pattern은 각자 timer를 유지하되 첫 spawn은 해금 뒤 0–500ms, 반복 spawn은 기준 interval ±500ms로 timing 전용 seeded RNG에서 다시 뽑는다. 공격 문구·방향 RNG와 기본 tool stream cadence는 바꾸지 않는다. onset은 최소 360ms 떨어뜨리고 서로 다른 active major family는 최대 세 개로 제한한다. approval도 같은 상한 안에서 다른 major와 겹칠 수 있다. 기준 logical arena 면적의 55%보다 작은 viewport는 spawn interval만 1.22배 늘린다.
