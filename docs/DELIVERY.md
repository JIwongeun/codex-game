...
- [x] Open Graph·Twitter 공유 카드가 favicon·Start header와 같은 original game mark 및 versioned image URL 사용
- [x] 조작법 표시
...
- [x] 기본 50%의 독립 SFX·BGM volume, hover·focus 사각 outline, `↑`·`↓` target 이동과 `←`·`→`·`A`·`D` 조절, M 입력과 Web Audio 전체 음소거·active tone 차단
