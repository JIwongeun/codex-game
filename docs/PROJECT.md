...
플레이어는 실제 커서처럼 보이는 캐릭터를 마우스로 움직인다. 화면 바깥에서 날아오는 요청의 진행 방향과 범위 공격의 예고 표시를 읽어 짧게 움직이며 피한다. 한 번 닿으면 run이 끝나고 생존 시간이 기록된다.
...
- 이동은 마우스 우선, WASD/방향키 대체 지원
- 시작·재시작은 클릭 또는 Space
- 종료 후 현재 생존 시간, 로컬 최고 기록, 회피 통계를 표시

### 이동과 판정

- 포인터가 플레이어와 떨어져 있으면 일정한 최대 속도로 포인터 방향을 따라간다.
- 포인터가 가까우면 멈춘다. 키보드를 놓아도 멈춘다.
- 플레이어는 화면 경계를 넘지 않는다.
- 보이는 커서 도형보다 실제 피격 반경을 작게 유지해 아슬아슬한 회피를 허용한다.
- `TAB` 공격은 화살촉에 해당하는 앞부분만 충돌한다.
...
1. `TAB STORM` — 시작부터 화면 사방에서 포인터를 향한 직선 요청이 날아온다.
2. `POP-UP` — 12초부터 붉은 진행선으로 경로를 예고한 뒤 빠르게 돌진한다.
3. `MEMORY LEAK` — 24초부터 원형 범위를 예고한 뒤 짧게 활성화한다.
4. `CONTEXT OVERFLOW` — 42초부터 화면을 가로지르는 수평 또는 수직 범위가 예고 후 활성화한다.
