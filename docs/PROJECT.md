...
플레이어 캐릭터를 따로 그리지 않는다. 브라우저 포인터가 플레이어이며, 게임 영역에서는 hard-edge 검은 pixel cursor로 보인다. CSS cursor hotspot과 게임의 충돌 좌표를 1:1로 일치시킨다. 화면 바깥에서 날아오는 요청의 진행 방향과 범위 공격의 예고 표시를 읽어 짧게 움직이며 피한다. 한 번 닿으면 run이 끝나고 생존 시간이 기록된다.
...
- 플레이어는 화면 경계를 넘지 않는다.
- 화면에는 플레이어 도형이나 피격점 표시를 전혀 그리지 않는다. 브라우저가 그리는 검은 custom cursor 자체만 플레이어로 보이며 입력 좌표와 시각 hotspot은 분리하지 않는다.
- 탭 blur/hidden pause 중에는 OS 기본 cursor를 사용하고 게임 좌표는 마지막 위치에 동결한다. 흐려진 장면 아래의 검은 cursor 위치로 돌아와 클릭해야 재개되며 다른 위치의 클릭과 Space는 재개하지 않는다.
...
- pause 중에는 동결된 검은 게임 cursor를 blur 아래에 남겨 돌아갈 위치를 보여 주고, 움직일 수 있는 OS 기본 cursor와 구분한다.
...
- custom black mouse cursor의 1:1 hotspot 이동과 터치 좌표 입력
...
- custom cursor hotspot 반경과 화면 경계 clamp
