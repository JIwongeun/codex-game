...
예고 단계는 항상 무해하고, 활성화 단계만 피격 또는 시야 차단을 발생시킨다. 특수 패턴은 각자 timer를 유지하되 첫 등장은 해금 뒤 0–1,000ms, 이후 반복은 매 기준 주기 ±1,000ms의 seeded timing jitter를 적용하고 실제 interval은 최소 800ms를 보장한다. `rm *`만 최소 1,600ms 간격을 사용하며 활성 square를 추가할 때도 일반 major round-robin 순서를 따른다. 기본 tool stream의 cadence는 유지하고, major onset은 최소 360ms 떨어뜨리며 서로 다른 active major family는 최대 세 개로 제한한다. 작은 logical viewport에서는 공격 속도를 낮추지 않고 생성 간격만 1.22배 늘린다. 공격 개체와 범위 수에는 별도 상한을 둔다.
...
- 사용자에게 보이는 완전한 화면은 Start와 Game 두 개뿐이다. Start는 최초 진입과 game over에서 `Esc`로 복귀할 때 사용하며 original context-loop game mark, 상단 background task 상태, 큰 `Codex is working.` headline, objective·control·fail state·last run·session best와 하나의 실행 CTA로 구성한다. 뒤에는 실제 공격과 같은 token 문법과 edge-to-edge 진행 방향을 가진 저대비 terminal·browser·Codex 문구가 느린 속도로 화면을 가로지른다. 피격 시 별도 Scene으로 이동하지 않고 Game을 100ms 멈춘 뒤 68% Canvas white wash와 25% DOM 순백 wash·1px blur·grayscale을 적용한다. contrast 필터와 Results global black hit flash는 사용하지 않아 빈 배경 면은 정확한 순백을 유지하고, frozen Game은 색 없이 약 24% 가시성으로 남긴다. 12×12 black player node와 기존 hit-source focus만 DOM foreground에서 같은 좌표와 크기로 다시 표시해 blur 없이 보존하고 공격 entity는 전부 blur 아래에 둔다. 중앙에는 danger red `GAME OVER`, 작은 `[context] overflow // exit code 1`, 충돌 원인, 클릭·Space Retry와 `Esc` Start 안내를 표시한다.
...
- Start ↔ Game 두 화면. 내부 `results` phase는 frozen Game 위의 `CONTEXT OVERFLOW` overlay이며 클릭·Space로 Retry, `Esc`로 last run이 반영된 Start에 복귀
