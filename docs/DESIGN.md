...
- Start에는 `SFX VOLUME`과 `BGM VOLUME` slider를 각각 한 줄로 둔다. 둘 다 기본 20%이며 이 위치가 이전 50%의 실제 출력과 같다. 100%의 channel gain은 이전 최대의 2.5배로 넓혀 20% 위에서 음량이 분명히 커지게 한다. 검정 1px rail과 검정색의 굵은 가로 직사각형 handle, Start의 monospace label과 검정 수치만 사용한다. 각 slider는 해당 음원군만 조절하고 `M`은 두 값을 보존한 채 전체를 즉시 음소거한다.
