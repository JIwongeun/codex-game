...
- Game over: 피격 뒤 simulation과 모든 presentation motion을 100ms 멈추고 Canvas 안에서 68% pure white로 frozen Game을 낮춘다. 그 위 DOM에는 25% pure-white wash와 1px blur·완전 grayscale만 적용한다. contrast 필터와 Results global black hit flash는 사용하지 않아 빈 배경 면은 정확한 순백으로 고정하고 배경 공격은 색 없이 약 24% 가시성으로 남긴다. 12×12 black player node와 기존 hit-source focus만 같은 logical 좌표·화면 scale의 DOM foreground에서 다시 표시해 blur 없이 보존하고, 공격 entity를 복제하는 foreground Canvas나 collision aperture는 사용하지 않는다. 중앙의 danger red `GAME OVER`는 한 번만 수평 조각으로 끊겨 나타난 뒤 `clip-path`를 완전히 해제해 마지막 glyph까지 보존하며, 그 아래 작은 monospace `[context] overflow // exit code 1`, run time, hit source와 `CLICK / SPACE RETRY`, `ESC START SCREEN`을 표시한다. 반복 glitch·CRT noise·지속 scanline은 사용하지 않는다. 새 session best이면 game over 시점에 green terminal success badge와 상승 2음 completion chime을 한 번 추가한다.
