...
- Stage 표시는 좌측 task status와 분리해 Game 화면 상단 정중앙에 둔다. Stage 10 진입 시 180ms 단발 흑백 반전 뒤 near-white red wash와 가장자리 danger frame·corner bracket·diagnostic tick을 유지하고 중앙 공격 영역에는 장식을 추가하지 않는다. Stage 10 label은 `CONTEXT // OVERFLOW`를 함께 표시하며 low alarm pulse를 BGM에 더한다.
