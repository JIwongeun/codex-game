...
- Stage 표시는 좌측 task status와 분리해 Game 화면 상단 정중앙에 둔다. Stage마다 fictional `SIM CTX`가 `1K`에서 `512K`까지 두 배로 증가하며, 실제 Codex session 수치가 아님을 명시한다. Stage 10 진입 시 180ms 단발 흑백 반전 뒤 near-white red wash와 가장자리 danger frame·corner bracket·diagnostic tick을 유지하고 중앙 공격 영역에는 장식을 추가하지 않는다. Stage 10 label은 `SIM CTX 512K // OVERFLOW`를 표시하며 low alarm pulse를 BGM에 더한다.
...
- `presentation/overflowPresentation.ts`: Stage 10 단발 transition과 지속 danger frame pulse를 elapsed time에서 계산
- `presentation/stagePresentation.ts`: Stage 1–10을 두 배로 증가하는 fictional `SIM CTX` label로 변환
